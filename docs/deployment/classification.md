...
- Navigate to the `YOLOv5-cls` folder and start the classification script by running:
    ...
    ??? bug "Image Not Found"
        ...
        This error can be caused by corrupt .jpg images, that are rarely
        generated by the automated monitoring script. Run the
        [`check_images.py`](https://github.com/maxsitt/insect-detect-ml/blob/main/check_images.py){target=_blank}
        script to find corrupted .jpg images in your data folder and move them to a new folder with:

        ``` powershell
        py insect-detect-ml-main/check_images.py -data insect-detect/data/
        ```

        After removing the corrupted .jpg images, the classification script should
        run without throwing an error now.
